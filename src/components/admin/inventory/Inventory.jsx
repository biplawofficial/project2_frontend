...
const Inventory = () => {
  ...
  const getInventoryData = async () => {
    try {
      ...
      const response = await axios.get(`https://project02-backend.onrender.com/getinventorydata`, {
        headers: {
          'Authorization': `${token}`
        }
      });
  ...
  const handleAdd = async(index) => {
    ...
    const token = localStorage.getItem('authToken');
    await axios.patch(`https://project02-backend.onrender.com/updateinventory`, updatedInventory[index], {
      headers: {
        'Authorization': `${token}`
      }
    });
  ...
  const handleImport = async(e) => {
    ...
    reader.onload = async (event) => {
      ...
      const token = localStorage.getItem('authToken');
      await axios.post(`https://project02-backend.onrender.com/bulkinventory`, {data:jsonData}, {
        headers: {
          'Authorization': `${token}`
        }
      });
